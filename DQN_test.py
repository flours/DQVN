...
#        self.output_dim = env.action_space.n
        ...
        self.q_network = self.ModelCreate(0)
        self.q_network2 = self.ModelCreate(1)
        self.q_network3 = self.ModelCreate(2)
        self.q_network4 = self.ModelCreate(3)
        self.t_network = self.ModelCreate(0)
        self.t_network2 = self.ModelCreate(1)
        self.t_network3 = self.ModelCreate(2)
        self.t_network4 = self.ModelCreate(3)

        #self.t_network.load_weights("weight.h5") # 初期の重みを考慮する場合
        self.q_network.summary()
        self.max_q = 0

    def ModelCreate(self,playerId):#ネットワーク構造
        #ip = Input(shape=(5,))
        #h = Reshape((5, 1, 1), input_shape = (5, 1))(ip)
        #h = Conv2D(32, (4,4),strides=(2,2),padding="same",activation='sigmoid')(h)
        #h = Dense(256, activation='relu')(h)
        #h = Conv2D(64,(4,4),strides=(4,4),padding="same",activation='sigmoid')(h)
        #h = Dense(512, activation='tanh')(h)
        #h = Conv2D(64,(3,3),strides=(3,3),padding="same",activation='sigmoid')(h)
        #h = Flatten()(h)
        #h = Dense(12)(h)
        ...
        model = Sequential()
        model.add(Flatten(input_shape=(1,) + DQN.observation_space.shape))
        model.add(Dense(2))
    ...
    def getAction(self, state, epsilon):
        ...
        if np.random.rand() < epsilon:
            return self.env.getRandomActions()

        #q_value = self.Predict(state)
        q_value = self.q_network.predict_on_batch(state)
        q1_value = self.q_network2.predict_on_batch(state)
        q2_value = self.q_network3.predict_on_batch(state)
        q3_value = self.q_network4.predict_on_batch(state)
        return np.argmax(q_value),np.argmax(q1_value),np.argmax(q2_value),np.argmax(q3_value)
...
def main():
    ...
    plot_model(agent.q_network, to_file='./model2.png',show_shapes=True)
